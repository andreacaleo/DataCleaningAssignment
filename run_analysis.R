...
mean_features_indices <- grep("mean",features_list$V2) #only count lowercase "mean", see CodeBook
...
#contains important features, no labels
...
train_activity_nos <- read.table("UCI HAR Dataset/train/y_train.txt") #"nos" stands for "numbers"
...
write.table(X_means, file = "tidy_data", row.names = F)
